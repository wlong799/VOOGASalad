...
public class InspectorSpriteView extends AbstractInspectorTabView {
	...
	private ComboBoxSettingsView myImageStyleBox, myPhysicsBox;
	private CheckBoxView myHerosCollisionCheckBox, myEnemiesCollisionCheckBox, myBlockCollisionCheckBox,
			myApplyPhysicsCheckBox, myReachPointCheckBox, enemyHasAIBox, myBounceHBox, myBounceVBox, myPushedHBox,
			myPushedVBox;
	...
	private void updateUI() {
		...
		myXBox = new TextInputBoxView(getController(), myLanguageResourceBundle.getString("posX"),
				mySprite.getPosition().getX() + "",
				(newVal) -> mySpriteView.setAbsolutePositionX(Double.parseDouble(newVal)));
		myYBox = new TextInputBoxView(getController(), myLanguageResourceBundle.getString("posY"),
				mySprite.getPosition().getY() + "",
				(newVal) -> mySpriteView.setAbsolutePositionY(Double.parseDouble(newVal)));
		myZBox = new TextInputBoxView(getController(), myLanguageResourceBundle.getString("posZ"),
				mySprite.getPosition().getZ() + "",
				(newVal) -> mySpriteView.setAbsolutePositionZ(Double.parseDouble(newVal)));

		myWidthBox = new TextInputBoxView(getController(), myLanguageResourceBundle.getString("width"),
				mySprite.getDimension().getWidth() + "",
				(newVal) -> mySpriteView.setDimensionWidth(Double.parseDouble(newVal), true));
		myHeightBox = new TextInputBoxView(getController(), myLanguageResourceBundle.getString("height"),
				mySprite.getDimension().getHeight() + "",
				(newVal) -> mySpriteView.setDimensionHeight(Double.parseDouble(newVal), true));
		myVelocityXBox = new TextInputBoxView(getController(), myLanguageResourceBundle.getString("initialVelX"),
				mySprite.getVelocity().getXVelocity() + "",
				(newVal) -> mySpriteView.getSprite().getVelocity().setXVelocity(Double.parseDouble(newVal)));
		myVelocityYBox = new TextInputBoxView(getController(), myLanguageResourceBundle.getString("initialVelY"),
		...
		myHerosCollisionCheckBox = new CheckBoxView(getController(),
		...
		myEnemiesCollisionCheckBox = new CheckBoxView(getController(),
		...
		myBlockCollisionCheckBox = new CheckBoxView(getController(),
		...
		Set<Class<? extends IPhysicsStrategy>> physics = reflections.getSubTypesOf(IPhysicsStrategy.class);
		...
		if (mySprite instanceof IBlock) {
			myReachPointCheckBox = new CheckBoxView(getController(), myLanguageResourceBundle.getString("assignGoal"),
					false, (obv, oldVal, newVal) -> {
		...
		if (mySprite instanceof Hero) {
			...
			myMaxJumpSlider = new SliderBoxView(getController(), myLanguageResourceBundle.getString("noJumps"),
					Double.parseDouble(componentProperties.getString("MIN_NUMBER_JUMPS")),
					Double.parseDouble(componentProperties.getString("MAX_NUMBER_JUMPS")), hero.getMaxNumberOfJumps(),
			...
			myInfiniteJumps = new CheckBoxView(getController(), myLanguageResourceBundle.getString("infiniteJumps"),
					false, (obv, oldVal, newVal) -> {
						if (newVal) {
			...
			myJumpUnitSlider = new SliderBoxView(getController(), myLanguageResourceBundle.getString("jumpUnit"),
					Double.parseDouble(componentProperties.getString("MIN_JUMP_UNIT")),
					Double.parseDouble(componentProperties.getString("MAX_JUMP_UNIT")), hero.getJumpingUnit(),
			...
			myPushedHBox = new CheckBoxView(
					...
					getController(), myLanguageResourceBundle.getString("enemyHorizontally"),
					hero.getPushByEnemyCollsionStrategy().getHorizontalBounce(), (obv, oldVal, newVal) -> {
						hero.getPushByEnemyCollsionStrategy().setMyHorizontalPushed(newVal);
					});
			myPushedVBox = new CheckBoxView(getController(), myLanguageResourceBundle.getString("enemyVertically"),
			...
			addSettingsViews(myActionView, myInfiniteJumps, myMaxJumpSlider, myJumpUnitSlider, myLivesConfiguringView,
					myHealthPointConfiguringView, myPushedHBox, myPushedVBox);
		}
		...
		if (mySprite instanceof Enemy) {
			...
			enemyHasAIBox = new CheckBoxView(getController(), myLanguageResourceBundle.getString("hasAI"),
					enemy.hasAI(), (obv, old_val, new_val) -> ((Enemy) mySprite).setHasAI(new_val));
			myPushedHBox = new CheckBoxView(getController(), myLanguageResourceBundle.getString("heroHorizontally"),
					enemy.getPushByHeroCollsionStrategy().getHorizontalBounce(), (obv, oldVal, newVal) -> {
						enemy.getPushByHeroCollsionStrategy().setMyHorizontalPushed(newVal);
					});
			myPushedVBox = new CheckBoxView(getController(), myLanguageResourceBundle.getString("heroVertically"),
					enemy.getPushByHeroCollsionStrategy().getVerticalBounce(), (obv, oldVal, newVal) -> {
						enemy.getPushByHeroCollsionStrategy().setMyVerticalPushed(newVal);
						;
					});
			addSettingsViews(enemyHasAIBox, myPushedHBox, myPushedVBox);
		}

		if (mySprite instanceof ICharacter) {
			ICharacter character = (ICharacter) mySprite;
			myDamageSlider = new SliderBoxView(getController(),
					...
					(obv, oldVal, newVal) -> character.getAttackByProjectileStrategy()
							.setDamageFromAllDirection(newVal.doubleValue()));
			myBounceHBox = new CheckBoxView(getController(), myLanguageResourceBundle.getString("bounceHorizontally"),
			...
			myBounceVBox = new CheckBoxView(getController(), myLanguageResourceBundle.getString("bounceVertically"),
			...
			addSettingsViews(myDamageSlider, myBounceHBox, myBounceVBox);
		}
		updateLayout();
	}
