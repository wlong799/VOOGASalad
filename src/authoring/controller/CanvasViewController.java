...
public class CanvasViewController {
	...
	/**
	 * refresh sprite views called after user selects a different level
	 */
	public void refresh() {
		initSpriteViews();
	}
	...
	/**
	 * method to add a SpriteView to canvas used for drag and drop from
	 * components view
	 *
	 * @param spView
	 * @param x
	 * @param y
	 * @param relative
	 *            if true, x and y are positions on screen instead of real
	 *            positions
	 */
	 ...
	 *            to be set
	 * @param x
	 *            new position X relative to top-left corner
	 * @param y
	 *            new position Y relative to top-left corner x and y are not
	 *            relative to the origin of content!
	 */
	...
	public void onDragSpriteView(SpriteView spView, MouseEvent event) {
		...
		this.setRelativePosition(spView, x - spView.getMouseOffset().getX(), y - spView.getMouseOffset().getY());
	}
	 ...
	 * @param endY
	 *            bottom right corner Y x, y are absolute sprite positions
	 */
	public void onResizeSpriteView(SpriteView spView, double startX, double startY, double endX, double endY) {
	...
	public void expand() {
		double newWidth = width() + UIConstants.SCREEN_CHANGE_INTERVAL;
		myEnvironment.getCurrentLevel().getLevelDimension().setWidth(newWidth);
		updateBackground();
	}

	public void shrink() {
		if (width() > UIConstants.CANVAS_STARTING_WIDTH) {
			double newWidth = width() - UIConstants.SCREEN_CHANGE_INTERVAL;
			myEnvironment.getCurrentLevel().getLevelDimension().setWidth(newWidth);
			updateBackground();
		}
	...
	public void taller() {
		double newHeight = height() + UIConstants.SCREEN_CHANGE_INTERVAL;
		myEnvironment.getCurrentLevel().getLevelDimension().setHeight(newHeight);
		updateBackground();
	}

	public void shorter() {
		if (height() > UIConstants.CANVAS_STARTING_HEIGHT) {
			double newHeight = height() - UIConstants.SCREEN_CHANGE_INTERVAL;
			myEnvironment.getCurrentLevel().getLevelDimension().setHeight(newHeight);
			updateBackground();
		}
	...
	private double width() {
		return myEnvironment.getCurrentLevel().getLevelDimension().getWidth();
	}

	private double height() {
		return myEnvironment.getCurrentLevel().getLevelDimension().getHeight();
	}
	...
	private void adjustScrollPane(double x, double y) {
		if (x < UIConstants.DRAG_SCROLL_THRESHOLD) {
			myScrollPane.setHvalue(Math.max(0, myScrollPane.getHvalue() - UIConstants.SCROLL_VALUE_UNIT));
		}
		if (scWidth - x < UIConstants.DRAG_SCROLL_THRESHOLD) {
			myScrollPane.setHvalue(Math.min(1, myScrollPane.getHvalue() + UIConstants.SCROLL_VALUE_UNIT));
		}
		if (y < UIConstants.DRAG_SCROLL_THRESHOLD) {
			myScrollPane.setVvalue(Math.max(0, myScrollPane.getVvalue() - UIConstants.SCROLL_VALUE_UNIT));
		}
		if (scHeight - y < UIConstants.DRAG_SCROLL_THRESHOLD) {
			myScrollPane.setVvalue(Math.min(1, myScrollPane.getVvalue() + UIConstants.SCROLL_VALUE_UNIT));
		}
	}

	/**
	 * @param data
	 *            if spriteViews get cleared also
	 */
	...
	private void updateBackground() {
		// TODO: 11/29/16 allow for tiling of multiple image paths, rather than
		// just first
		...
		if (myEnvironment.getCurrentLevel().getBackground().getImagePaths().size() == 0) {
			...
			imageView.setViewport(
					new Rectangle2D(0, 0, (1 - ((usedWidth - width) / adjustedWidth)) * backgroundImage.getWidth(),
