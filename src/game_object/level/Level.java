...
public class Level implements ILevelVisualization {
    ...
    public Level (Game parentGame, String id) {
        ...
        myDimension = new Dimension(DEFAULT_DIMENSION_WIDTH, DEFAULT_DIMENSION_HEIGHT);
    ...
    public void addSprite (ISprite sprite) {
        ...
        if (sprite.getPosition().getX() + sprite.getDimension().getWidth() > this.myDimension
                .getWidth()) {
            myDimension.setWidth(sprite.getPosition().getX() + sprite.getDimension().getWidth());
        }
        if (sprite.getPosition().getY() + sprite.getDimension().getHeight() > myDimension
                .getHeight()) {
    ...
    public ActionTrigger getTriggerWithSpriteAndAction (
        ...
        for (ActionTrigger trigger : myTriggers) {
            if (trigger.getSprite().equals(sprite) &&
    ...
    public void update () {
        ...
        allSprites.sort( (s1, s2) -> s1.getPosition().getZ() > s2.getPosition().getZ() ? 1 : -1);
